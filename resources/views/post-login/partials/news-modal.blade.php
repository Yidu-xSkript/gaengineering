<div class="modal modal-edu-general Customwidth-popup-WarningModal fade" role="dialog" id=<?= 'edit-news'.$news->id ?>>
    <div class="modal-dialog" style="max-width: 50%;">
        <div class="modal-content">
            ...
            <form action="/auth/news/{{ $news->id }}/edit" method="post" enctype="multipart/form-data">
                @csrf
                @METHOD('PATCH')
                <div class="modal-body">
                    <div class="form-group">
                        <label for="name" style="font-size: 15px;">Title <small style="color: red;">*</small></label>
                        <input type="text" class="form-control" value="{{ $news->title }}" name="title" id="name" placeholder="Title" style="font-size: 15px;" required="">
                    </div>

                    <div class="form-group">
                        <label for="slug" style="font-size: 15px;">Slug/Description about the News<small style="color: red;"></small>*</small></label>
                    ...
                    @if(!is_null($news->image_url))
                        <img src="{{ $news->image_url }}" class="img-card" style="width: 100%; height: 400px; object-fit: cover;">
                    @endif
                </div>
...
<div class="modal fade" role="dialog" style="border: none;" id=<?= 'delete-news'.$news->id; ?>>
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="/auth/news/{{$news->id}}/destroy" method="post">
                @csrf
                @method('DELETE')
