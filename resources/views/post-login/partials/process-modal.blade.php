<div class="modal modal-edu-general Customwidth-popup-WarningModal fade" role="dialog" id=<?= 'edit'.$process->id ?>>
    <div class="modal-dialog" style="max-width: 50%;">
        <div class="modal-content">
            ...
            <form action="/auth/work-process/{{ $process->id }}/edit" method="post" enctype="multipart/form-data">
                @csrf
                @METHOD('PATCH')
                <div class="modal-body">
                    <div class="form-group">
                        <label for="name" style="font-size: 15px;">Title <small style="color: red;">*</small></label>
                        <input type="text" class="form-control" value="{{ $process->title }}" name="title" id="name" placeholder="Title" style="font-size: 15px;" required="">
                    </div>

                    <div class="form-group">
                        <label for="slug" style="font-size: 15px;">Slug/Description<small style="color: red;"></small>*</small></label>
                        <textarea class="form-control slug"  rows="8" name="slug" id="slug_modal" placeholder="Slug / Description" style="font-size: 15px;" required><?= $process->slug;?></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="image_position" class="form-control" style="font-size: 17px;" name="image_position" required>
                            ...
                            <option value="left" @if($process->image_position == "left") selected @endif>Left</option>
                            <option value="right" @if($process->image_position == "right") selected @endif>Right</option>
                        </select>
                    </div>
                    ...
                    @if(!is_null($process->image_url))
                        <img src="{{ $process->image_url }}" class="img-card" style="width: 100%; height: 400px; object-fit: cover;">
                    @endif
                </div>
...
<div class="modal fade" role="dialog" style="border: none;" id=<?= 'delete'.$process->id; ?>>
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="/auth/work-process/{{$process->id}}/destroy" method="post">
                @csrf
                @method('DELETE')
